 ...
 */
package com.kuaishouaxiu.www.kuaishouaxiu.view.pullview;
...
public class AbPullToRefreshView extends LinearLayout {
    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        Boolean x = isRecyclerViewMove(deltaY);
        if (x != null) return x;

        return false;
    }

    private Boolean isRecyclerViewMove(int deltaY) {
        if (deltaY > 0) {//下
            //获取第一个可见view的位置
            int firstItemPosition = getRecyclerViewItemPosition(true);
            if (!mEnablePullRefresh) {
                return false;
            }
            ...
            //获取最后一个可见view的位置
            int lastItemPosition = getRecyclerViewItemPosition(false);
            // 判断是否禁用上拉加载更多操作
            if (!mEnableLoadMore) {
                return false;
            }
            ...
            // 等于父View的高度说明mAdapterView已经滑动到最后
            if (/*lastChild.getBottom() <= getHeight()
                            && */lastItemPosition == mRecyclerView
    ...
    private int getRecyclerViewItemPosition(boolean isFirst) {
        ...
        if (layoutManager instanceof StaggeredGridLayoutManager) {
            ...
            frist = findMin(lastPositions);
            last = findMax(lastPositions);
        }
        return isFirst ? frist : last;
    }
